...
class Block(nn.Module):
    ...
    def forward(self, x, layer_past=None, return_present=False, mask=None):
        # TODO: check that training still works
        if return_present: assert not self.training
        # layer past: tuple of length two with B, nh, T, hs
        attn, present = self.attn(self.ln1(x), layer_past=layer_past, mask=mask)
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size, block_size, n_layer=12, n_head=8, n_embd=256,
                 embd_pdrop=0., resid_pdrop=0., attn_pdrop=0., n_unmasked=0, mask_token=False):
        super().__init__()
    ...
    def forward_with_mask(self, idx, mask=None, embeddings=None, targets=None):
        # forward the GPT model
        token_embeddings = self.tok_emb(idx) # each index maps to a (learnable) vector
